...
fn test_apple_jwt_creation_with_crypto_module() {
    ...
    assert!(
        result.is_ok(),
        "Apple JWT generation should succeed: {:?}",
        result.as_ref().err()
    );
    ...
    assert!(
        header_result.is_ok(),
        "Should be able to decode JWT payload"
    );
    ...
    assert_eq!(
        ...
        "Audience should be Apple ID"
    );

    // Verify timestamps are present and reasonable
    assert!(
        payload["iat"].is_number(),
        "Issued at timestamp should be present"
    );
    assert!(
        payload["exp"].is_number(),
        "Expiration timestamp should be present"
    );
...
fn test_apple_jwt_invalid_key_file() {
    ...
    assert!(result
        .unwrap_err()
        .contains("Failed to read Apple private key file"));
}
...
fn test_apple_jwt_deterministic_with_same_input() {
    ...
    assert_ne!(
        ...
        "Issue times should differ"
    );
    assert_ne!(
        payload1["exp"], payload2["exp"],
        "Expiry times should differ"
    );

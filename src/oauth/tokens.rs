//! Token processing and validation for OAuth module
//!
//! This module handles ID token decoding, validation, and claim extraction
//! for OAuth authentication flows without depending on session types.
...
/// Process ID token and return OAuth result - NO session creation logic here
///
/// This function extracts claims from ID tokens and returns an `OAuthResult`.
///
/// # Errors
///
/// Returns an error if:
...
pub fn process_id_token(
    ...
    let user_email = extract_email(&claims)
        .or_else(|| {
    ...
    let mut user_name = extract_name(&claims);
    ...
    let created_at = extract_issued_at(&claims);
    let normalized_provider = normalize_provider(provider, &claims);
    ...
    // Return OAuthResult directly - NO session creation here
...
mod tests {
    ...
    fn test_extract_subject_success() {
        ...
        assert_eq!(extract_subject(&claims).unwrap(), "12345");
    }
    ...
    fn test_extract_subject_missing() {
        ...
        assert!(extract_subject(&claims).is_err());
    }
    ...
    fn test_extract_name_missing() {
        ...
        assert_eq!(extract_name(&claims), None);
    }

...
async fn main() -> std::io::Result<()> {
    ...
    let settings = VouchrsSettings::load()
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, format!("Failed to load settings: {}", e)))?;
    ...
    oauth_config.initialize_from_settings(&settings).await
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, format!("Failed to initialize OAuth providers: {}", e)))?;
    ...
    HttpServer::new(move || {
        ...
        let cors = Cors::default()
            .allowed_origin_fn(move |origin, _| {
                cors_origins.iter().any(|allowed| allowed == origin.to_str().unwrap_or(""))
            })
...
fn print_startup_info(bind_address: &str, session_backend: &str, settings: &VouchrsSettings) {
    ...
    println!("  {}/oauth2/callback", settings.application.redirect_base_url);
    println!();
    ...
    println!("  Static files folder: {}", settings.static_files.assets_folder);
}
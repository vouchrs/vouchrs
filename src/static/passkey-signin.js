 ...
 * - Automatic detection of available passkeys
            ...
            if (PublicKeyCredential.parseRequestOptionsFromJSON) {
                ...
                } catch (e) {ross-platform passkey authentication (Chrome, Safari, 1Password, Bitwarden, etc.)
...
async function authenticateWithPasskey() {
    ...
    try {
        ...
        try {
            ...
            if (error.status === 404 || error.message?.includes('no passkeys') || error.error?.includes('no passkeys')) {
                ...
                setTimeout(() => {
                    ...
                    window.location.href = '/oauth2/static/passkey-register.html?from=signin&reason=no-passkeys';
                }, 1500);
                return;
            }
            throw error;
        ...
        if (platform.isDesktop) {
            ...
            if (PublicKeyCredential.parseRequestOptionsFromJSON) {
                try {
                    options = PublicKeyCredential.parseRequestOptionsFromJSON(_options.request_options.publicKey);
                } catch (e) {
        ...
        if (error.name === 'NotAllowedError') {
            ...
            setTimeout(() => {
                window.location.href = '/oauth2/static/passkey-register.html?from=signin&reason=cancelled';
            }, 2000);
            return;
